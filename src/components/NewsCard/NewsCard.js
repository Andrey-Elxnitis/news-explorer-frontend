...
function NewsCard(props) {
  ...
  const button = `${pathname === '/saved-news' ? 'news-card__button-delete' : 'news-card__button'}`;

  // если вкладка сохраненные статьи, отображаем ключевое слово на статье
  const keyword = `${pathname === '/saved-news' ? 'news-card__keyword news-card__keyword_active' : 'news-card__keyword'}`;
